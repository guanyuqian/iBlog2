...
!function ($) {
     ...
     */
    bTabs.prototype.addTab = function (id, title, url, callback) {
        ...
        openTabs.push(tabId);
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $(content).append(data).each(callback);
    ...
    bTabs.prototype.closeTab = function (id) {
        ...
        //删除回调函数
        if(typeof deleteScenicCallback  != 'undefined' && deleteScenicCallback  instanceof Function) {//函数是否存在

           deleteScenicCallback(id);
        }
        if (openTabs && $.isArray(openTabs) && openTabs.length > 0) {
            var index = -1;
     ...
     * 新增标签页
     */
    function bTabsAdd(id, title, url,callback) {
        return this.each(function () {
            if (!id || !title || !url) return;
            var $this = $(this), data = $this.data('bTabs');
            if (data)
                data.addTab(id, title, url, callback);
        });
    }
    /**
     * 关闭标签页
